...
function renderSteps(){const icons=['👤','👥','🧭','🏨','🛂','✈️','🚌','🧩']; $('steps').innerHTML=stepNames.map((x,i)=>`<button type="button" class="step ${i===step?'active':''}" onclick="goBookingStep(${i})">${icons[i]} ${i+1}. ${x}</button>`).join('') }
...
function renderWizard(){let w=$('wizard');if(!q)return;normalizeBooking();if(step===-1){$('steps').innerHTML='';w.innerHTML=`<div class="choicegrid"><div class="choicecard" onclick="startFullBooking()"><div class="choiceicon">🎟️</div><h3>New Booking</h3><p>Create a complete booking with passenger details, hotels, visa, flights, transport, services and an automatically generated voucher.</p><button class="btn gold" style="margin-top:8px">Start New Booking</button></div><div class="choicecard" onclick="newPackageCost()"><div class="choiceicon">🧮</div><h3>Calculate the Package</h3><p>Quickly calculate visa, hotel accommodation and ticket cost for adults, children with/without bed and infants.</p><button class="btn primary" style="margin-top:8px">Open Package Cost Q</button></div></div>`;return;}renderSteps();if(!q)return;
...
function hotelRate(s){let h=hotelObj(s);if(!h)return 0;let r=h[4+roomTypes.indexOf(s.room)];return r==null?0:r}
...
function pcRate(city,name,room){let h=pcHotelObj(city,name);if(!h)return 0;let r=h[4+roomTypes.indexOf(room)];return r==null?0:r}
