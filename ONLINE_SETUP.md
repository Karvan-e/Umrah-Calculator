# V16.7 Online Setup / Hotel Catalog Update
...
The existing existing V15 functions `verify_password_reset` and `get_login_email_by_username` must also exist.
...
Run `V16_4_SECURITY_HARDENING.sql` after the existing tables/functions are present. It adds owner/Super Admin RLS enforcement for sales, shared app configuration, vouchers and Package Cost Q records. Ordinary users must never be able to read another user's sales or saved records through the API.

V16.7 also enforces that hotel accommodation is available only when the selected visa type is Umrah Visa. Only Visa, Visit Visa, Work Visa and Study Visa do not add hotel charges.
